...
class ProfilScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return SafeArea(child: SingleChildScrollView(child: Column(children: [
      Container(color: AppTheme.white, padding: const EdgeInsets.fromLTRB(20,20,20,20),
        child: Obx(() => Row(children: [
          Container(width: 60, height: 60,
              decoration: BoxDecoration(shape: BoxShape.circle,
                  gradient: LinearGradient(colors: [AppTheme.primaryBlue, AppTheme.deepBlue],
                      begin: Alignment.topLeft, end: Alignment.bottomRight)),
              child: Center(child: Text(
                  ctrl.nama.value.isNotEmpty ? ctrl.nama.value[0].toUpperCase() : 'U',
                  style: const TextStyle(fontSize: 24, fontWeight: FontWeight.w700,
                      color: Colors.white, fontFamily: 'Poppins')))),
          const SizedBox(width: 14),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(ctrl.nama.value, style: const TextStyle(fontSize: 16,
          ...
          GestureDetector(
            ...
            child: Container(width: 36, height: 36,
                ...
                child: const Icon(Icons.edit_outlined, size: 16, color: AppTheme.textGrey)),
          ),
        ]))),
      ...
      Padding(padding: const EdgeInsets.symmetric(horizontal: 20), child: Column(children: [
        _MenuCard(items: [
          ...
          _MI(Icons.support_agent_rounded, const Color(0xFF06B6D4), 'Customer Services',
              () => Get.to(() => const CustomerServicesScreen())),
        ]),
        const SizedBox(height: 12),
        _MenuCard(items: [
          _MI(Icons.logout_rounded, const Color(0xFFEF4444), 'Keluar Akun',
              () => Get.find<ProfilController>().showLogoutDialog(), destructive: true),
        ]),
...
class _MI {
  ...
  _MI(this.icon, this.color, this.label, this.onTap, {this.destructive = false});
}
        ...
        return Column(children: [
          ListTile(
            ...
            title: Text(item.label, style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500,
                color: item.destructive ? const Color(0xFFEF4444) : AppTheme.textDark, fontFamily: 'Poppins')),
            trailing: item.destructive ? null
                : const Icon(Icons.chevron_right, color: AppTheme.textGrey, size: 20),
            contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 2),
          ),
          if (i < items.length - 1) const Divider(height: 1, color: AppTheme.divider, indent: 68),
        ]);
      }).toList()),
    );
  }
...
class EditProfilScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(backgroundColor: AppTheme.offWhite, appBar: _appBar('Edit Profile'),
      body: SingleChildScrollView(padding: const EdgeInsets.all(20), child: Column(children: [
        Center(child: Stack(children: [
          Obx(() => Container(width: 80, height: 80,
              decoration: BoxDecoration(shape: BoxShape.circle,
                  gradient: LinearGradient(colors: [AppTheme.primaryBlue, AppTheme.deepBlue],
                      begin: Alignment.topLeft, end: Alignment.bottomRight)),
              child: Center(child: Text(
        ...
        _card([
          ...
          _field('Email', ctrl.emailCtrl, 'Email', kb: TextInputType.emailAddress),
          _field('Nomor Telepon', ctrl.teleponCtrl, '+62 xxx', kb: TextInputType.phone),
          Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('Jenis Kelamin', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w500,
...
class PusatBantuanScreen extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Scaffold(backgroundColor: AppTheme.offWhite, appBar: _appBar('Pusat Bantuan'),
      body: SingleChildScrollView(padding: const EdgeInsets.all(20), child: Column(
        crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Text('Pertanyaan Umum', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600,
            color: AppTheme.textDark, fontFamily: 'Poppins')),
        const SizedBox(height: 12),
        Container(decoration: BoxDecoration(color: AppTheme.white, borderRadius: BorderRadius.circular(16),
            border: Border.all(color: AppTheme.inputBorder)),
          child: Column(children: _faqs.asMap().entries.map((e) => Column(children: [
            _FaqTile(q: e.value.$1, a: e.value.$2),
            if (e.key < _faqs.length - 1) const Divider(height: 1, color: AppTheme.divider),
          ])).toList())),
      ])));
  }
}
...
class _FaqTileState extends State<_FaqTile> {
  ...
  @override
  Widget build(BuildContext context) {
    return AnimatedSize(duration: const Duration(milliseconds: 230), child: GestureDetector(
      onTap: () => setState(() => _open = !_open), behavior: HitTestBehavior.opaque,
      child: Padding(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 14),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            Expanded(child: Text(widget.q, style: TextStyle(fontSize: 13,
                fontWeight: _open ? FontWeight.w600 : FontWeight.w500,
                color: AppTheme.textDark, fontFamily: 'Poppins'))),
            const SizedBox(width: 8),
            AnimatedRotation(duration: const Duration(milliseconds: 200), turns: _open ? 0.5 : 0,
                child: const Icon(Icons.keyboard_arrow_down_rounded,
                    color: AppTheme.textGrey, size: 20)),
          ]),
          if (_open) ...[const SizedBox(height: 10),
            Text(widget.a, style: AppTheme.body.copyWith(fontSize: 12, height: 1.6))],
        ]))));
  }
}
...
class PengingatScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final ctrl = Get.find<ProfilController>();
    return Scaffold(backgroundColor: AppTheme.offWhite, appBar: _appBar('Pengingat'),
      body: Padding(padding: const EdgeInsets.all(20), child: Column(
        ...
        _sh('Pelacakan Tujuan'), const SizedBox(height: 8),
        _toggleCard([
          (Icons.shield_outlined, const Color(0xFFEF4444), 'Batas Anggaran',
           'Peringatan batas pengeluaran', ctrl.batasAnggaran, ctrl.toggleBatasAnggaran),
          (Icons.bar_chart_rounded, const Color(0xFF8B5CF6), 'Laporan Mingguan',
           'Rangkuman pengeluaran Anda', ctrl.laporanMingguan, ctrl.toggleLaporanMingguan),
        ]),
      ])));
  }

  Widget _sh(String t) => Text(t, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600,
      color: AppTheme.textGrey, fontFamily: 'Poppins'));

  Widget _toggleCard(List<(IconData, Color, String, String, RxBool, Function(bool))> items) {
    return Container(decoration: BoxDecoration(color: AppTheme.white,
        borderRadius: BorderRadius.circular(16), border: Border.all(color: AppTheme.inputBorder)),
      child: Column(children: items.asMap().entries.map((e) {
        ...
        return Column(children: [
          ...
          if (i < items.length - 1) const Divider(height: 1, color: AppTheme.divider, indent: 66),
        ]);
      }).toList()));
  }
}
...
class CustomerServicesScreen extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Scaffold(backgroundColor: AppTheme.offWhite, appBar: _appBar('Customer Services'),
      body: Padding(padding: const EdgeInsets.all(20),
        child: _MenuCard(items: [
          _MI(Icons.chat_bubble_outline_rounded, const Color(0xFF22C55E), 'Hubungi Admin', () {
          ...
          _MI(Icons.info_outline_rounded, AppTheme.primaryBlue, 'Salinan Dompetin', () {
            Get.snackbar('Dompetin', 'Versi 1.0.0 - Personal Finance Manager',
                snackPosition: SnackPosition.TOP, margin: const EdgeInsets.all(16));
          }),
        ])));
  }
...
class KeamananAkunScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final ctrl = Get.find<ProfilController>();
    return Scaffold(backgroundColor: AppTheme.offWhite, appBar: _appBar('Keamanan Akun'),
      body: Padding(padding: const EdgeInsets.all(20),
        child: _MenuCard(items: [
          _MI(Icons.lock_outline_rounded, AppTheme.primaryBlue, 'Ganti Kata Sandi',
              () => Get.to(() => const GantiKataSandiScreen())),
          _MI(Icons.delete_outline_rounded, const Color(0xFFEF4444), 'Hapus Akun',
              ctrl.showHapusAkunDialog, destructive: true),
        ])));
  }
}
...
class GantiKataSandiScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final ctrl = Get.find<ProfilController>();
    return Scaffold(backgroundColor: AppTheme.offWhite, appBar: _appBar('Ganti Kata Sandi'),
      body: Padding(padding: const EdgeInsets.all(20), child: Column(children: [
        _card([
          _pwField('Kata Sandi', ctrl.kataSandiCtrl, 'Kata Sandi',
              ctrl.isKataSandiVisible, ctrl.toggleKataSandi),
          const SizedBox(height: 14),
          _pwField('Konfirmasi Kata Sandi', ctrl.konfirmasiCtrl, 'Ulangi Kata Sandi',
              ctrl.isKonfirmasiVisible, ctrl.toggleKonfirmasi),
        ]),
        const SizedBox(height: 24),
        Obx(() => SizedBox(width: double.infinity, height: 52, child: ElevatedButton(
          onPressed: ctrl.isSavingPassword.value ? null : ctrl.gantiKataSandi,
          style: ElevatedButton.styleFrom(backgroundColor: AppTheme.primaryBlue,
              shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)), elevation: 0),
          child: ctrl.isSavingPassword.value
              ? const SizedBox(width: 20, height: 20,
                  child: CircularProgressIndicator(strokeWidth: 2.5, color: Colors.white))
              : const Text('Perbaru Kata Sandi', style: TextStyle(fontSize: 16,
...
Widget _field(String label, TextEditingController ctrl, String hint,
  ...
  Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    Text(label, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w500,
        color: AppTheme.textDark, fontFamily: 'Poppins')),
    const SizedBox(height: 6),
    TextField(controller: ctrl, keyboardType: kb, style: AppTheme.label,
...
Widget _pwField(String label, TextEditingController ctrl, String hint,
  ...
  Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    ...
    const SizedBox(height: 6),
    Obx(() => TextField(
      controller: ctrl, obscureText: !visible.value, style: AppTheme.label,
      decoration: AppTheme.inputDecoration(hint: hint,
          suffixIcon: IconButton(
            icon: Icon(visible.value ? Icons.visibility_outlined : Icons.visibility_off_outlined,
                color: AppTheme.textGrey, size: 20),
            onPressed: toggle)),
    )),
  ]);
